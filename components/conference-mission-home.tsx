...
import { ConferenceMeetingSessions } from "@/components/conference-meeting-sessions"
    ...
    <main className="relative z-0 min-h-svh bg-[#0458e2]">
      ...
      <div className="mx-auto max-w-2xl">
        <div className="px-[6%] pt-6 sm:px-8">
          ...
          {/* 3 場聚會全部隨時可以點進去（不再有「還沒輪到不能看」的時間限制），
              但首頁不想一次塞 3 張大圖：只放大張的「即將開始」卡片，其餘場次
              收在「查看所有活動」展開清單裡（見 conference-meeting-sessions.tsx，
              跟 CAMP 首頁同一套模式）。視覺圖依場次換成 session.image（還沒拿到
              真圖的場次先共用佔位圖，見 lib/opening-conference-content.ts）。 */}
          <ConferenceMeetingSessions
            nextSession={nextSession}
            allSessions={conferenceSessions}
