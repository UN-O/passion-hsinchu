...
import { claimIdentity, type ClaimState } from "./actions"

const initialState: ClaimState = { error: null }

export function ClaimForm({ churches }: { churches: string[] }) {
  const [state, formAction, isPending] = useActionState(claimIdentity, initialState)
  ...
  if (churches.length === 0) {
    return (
      <p className="mt-8 text-sm text-muted-foreground">名冊尚未匯入，請洽現場工作人員。</p>
    )
  }
